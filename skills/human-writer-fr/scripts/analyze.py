#!/usr/bin/env python3
"""
human-writer-fr / analyze.py — deterministic AI-tell detector (French).

French specialization of the master human-writer analyzer. Differences vs the
master bilingual analyzer (everything else is identical):
  - --lang choices=["fr"].
  - detect_tricolons uses the French conjunction (et); no lang parameter.
  - NEW detect_accent_stripping: flags prose whose accented-character ratio is
    far below the native French floor (an entire file emitted without
    é è ê ë à â ä î ï ô ö ù û ü ÿ ç œ æ reads as broken French — a frequent
    AI/plain-text failure mode). The accent class includes the UPPERCASE forms
    so all-caps de-accented headings are also counted. Wired as a low-weight
    branch; guarded against false positives on very short input (≥40-word floor).
    Ported from human-writer-pt.
...
def count_sentences(text: str) -> int:
    # French sentence terminators are [.!?] — same as the master. The
    # accent-stripping check (a per-character ratio) does not affect sentence
    # splitting at all.
    ...
    if em and em.get("flag"):
        ...
        recs.append({
            ...
            "action": (
                ...
                "colons, parentheses, or full sentences (French prose rarely uses '—')."
            ),
        })

    acc = styl.get("accent_stripping")
    if acc and acc.get("flag"):
        recs.append({
            "priority": "high" if acc.get("severity") == "high" else "medium",
            "action": (
                f"Restore French accents: accented-char ratio is {acc.get('value')} "
                f"(floor {acc.get('threshold')}). De-accented prose (probleme for 'problème', "
                "fonctionnalite for 'fonctionnalité', etre for 'être') reads as broken French. "
                "Re-accentuate é è ê ë à â ä î ï ô ö ù û ü ÿ ç œ before publishing."
            ),
        })
...
def compute_score(tells: dict, weights: dict) -> int:
    ...
    if tricolons >= 3:
        stylistic_pts += 5
    # NEW: accent-stripping typography tell — SMALL low-weight branch (FR).
    # A file emitted without é è ê à â î ï ô ù ç … reads as broken French; the
    # accented-char ratio collapsing far below the native floor is a real surface
    # signature. It contributes only a few points (it is one binary signal, not a
    # per-occurrence count) and never dominates the score.
    acc = styl.get("accent_stripping")
    if acc and acc.get("flag"):
        stylistic_pts += 5 if acc.get("severity") == "high" else 3
...
# French tricolon: "X, Y et Z" / "X, Y, et Z". The conjunction is always `et`
# in French. Matches the comma-comma-conjunction rhythm the same way the master
# detectors do.
TRICOLON_FR = re.compile(r"\b\w+,\s*\w+,?\s*et\s+\w+\b", flags=re.IGNORECASE)


def detect_tricolons(text: str) -> int:
    return len(TRICOLON_FR.findall(text))


# The accented characters native French prose carries: the acute/grave/
# circumflex/diaeresis vowels, the cedilla, and the œ/æ ligatures — both cases,
# so an all-caps de-accented heading ("PROBLEME" for "PROBLÈME") is also counted.
# These are the surface that disappears when AI/plain-text output strips accents
# ("problème" → "probleme", "fonctionnalité" → "fonctionnalite", "être" → "etre").
_ACCENTED_FR_RX = re.compile(
    r"[éèêëàâäîïôöùûüÿçœæÉÈÊËÀÂÄÎÏÔÖÙÛÜŸÇŒÆ]"
)
# Count *letters* as the denominator. \w includes accented letters so they are
# in the denominator; we exclude digits and underscore.
_LETTER_RX = re.compile(r"[^\W\d_]", flags=re.UNICODE)
# Minimum word count before the detector is allowed to flag. Accents are sparse
# in French (native ratio ≈ 0.033–0.039), so a short snippet can legitimately
# carry zero accented characters; flagging it would be a false positive.
ACCENT_MIN_WORDS = 40


def detect_accent_stripping(text: str, threshold: float) -> dict:
    """French typography tell (ported from human-writer-pt): an entire file
    emitted with its accents stripped reads as broken French on sight.

    We compute the ratio of accented French characters
    (`[éèêëàâäîïôöùûüÿçœæ…]`, both cases) to total letters. Native French prose
    sits around 0.033–0.039; a value far below the floor `threshold` means the
    accents were dropped. The detector flags only when the ratio is below the
    floor AND the text is long enough (≥ ACCENT_MIN_WORDS words) for the ratio to
    be meaningful — guarding against false positives on short, legitimately
    accent-sparse snippets.

    Calibrated NOT to fire on clean native prose (which always carries accents).
    """
    word_count = len(re.findall(r"\b\w+\b", text))
    letters = len(_LETTER_RX.findall(text))
    accented = len(_ACCENTED_FR_RX.findall(text))
    ratio = (accented / letters) if letters else 0.0
    long_enough = word_count >= ACCENT_MIN_WORDS
    flag = long_enough and ratio < threshold
    # Severity scales with how far below the floor we are. A ratio at/below half
    # the floor (e.g. a fully de-accented file at ratio 0.0) is the high case.
    if flag and ratio < threshold / 2:
        severity = "high"
    elif flag:
        severity = "medium"
    else:
...
def analyze(text: str, lang: str, content_type: str) -> dict:
    ...
    accent = detect_accent_stripping(text, thresholds["min_accent_ratio"])
...
def render_human(result: dict) -> str:
    lines: list[str] = []
    lines.append("=" * 60)
    lines.append(f"  human-writer-fr / analyze.py — {result.get('input_file', '<stdin>')}")
    lines.append("=" * 60)
    lines.append(
    ...
    acc = styl.get("accent_stripping") or {}
    acc_flag = "FLAG" if acc.get("flag") else "ok"
    lines.append(
        f"accent ratio : {acc.get('value', 0)} "
        f"({acc.get('accented_chars', 0)}/{acc.get('letters', 0)} letters, "
        f"floor>={acc.get('threshold', 0)})  [{acc_flag}]"
    )
...
def main() -> int:
    p = argparse.ArgumentParser(description="Deterministic AI-tell analyzer (French)")
    p.add_argument("--input", type=Path, help="Input file (markdown/txt). Stdin if omitted.")
    p.add_argument("--lang", choices=["fr"], required=True)
